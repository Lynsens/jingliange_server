...
// @Failure 500 {object} app.Response "{"code":500,"msg":"internal server error","data":null}"
// @Router /api/admin/deleteMenuItem/{id} [delete]
func DeleteMenuItem(c *gin.Context) {
	...
	idStr := c.Param("id")
	if idStr == "" {
		appG.Response(http.StatusBadRequest, e.INVALID_PARAMS, "Menu ID is required")
		return
	}
	...
	// 检查菜品是否存在且状态为正常
	existingMenu, err := repo.GetMenuByID(id)
	if err != nil {
	...
	// 执行软删除
	if err := repo.DeleteMenu(id); err != nil {
		appG.Response(http.StatusInternalServerError, e.ERROR, nil)
		return
	}
